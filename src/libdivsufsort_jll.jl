...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import libdivsufsort_jll on Julia versions older than 1.3!")
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("3acd9466-597a-52f5-9fc8-ad906e2116c6"))
platforms = [Pkg.Artifacts.unpack_platform(e, "libdivsufsort", artifacts_toml) for e in artifacts["libdivsufsort"]]
...
if best_platform === nothing
    @debug("Unable to load libdivsufsort; unsupported platform $(triplet(platform_key_abi()))")
else
    # Load the appropriate wrapper
    include(joinpath(@__DIR__, "wrappers", "$(best_platform).jl"))
end

end  # module libdivsufsort_jll
